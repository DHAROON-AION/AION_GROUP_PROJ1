...
## Project at a Glance
A six-week internship sprint (Jun 23 ΓÇô Aug 04, 2026) in which five interns build, evaluate, and document an open-source, fully self-hosted AI stack for a GCC bank ΓÇö covering local LLM serving, agents, retrieval-augmented generation, document intelligence, observability, and guardrails.
...
This project, AION ΓÇô AI Factory, is a structured six-week internship sprint in which five interns learn to design, build, and evaluate an AI stack made entirely of open-source components that can be self-hosted on the bank's own infrastructure ΓÇö with no cloud API key required at any point.
...
| Field | Description |
| :--- | :--- |
| **Interns AΓÇôE** | Five summer interns, each owning one layer of the stack end-to-end. |
...
* A local LLM served via Ollama (Llama 3 or Qwen) ΓÇö no cloud key needed to start building.
* A shared ΓÇ£hello agentΓÇ¥ starter template that everyone forks, so nobody begins from a blank file.
...
### Track A ΓÇö Models & Serving (FR-A.x)
...
### Track B ΓÇö Agents & Tool Access (FR-B.x)
...
### Track C ΓÇö Retrieval, RAG & Chat (FR-C.x)

| ID | Requirement | Acceptance criteria |
| :--- | :--- | :--- |
| **FR-C.1** | The system shall answer questions grounded in the bank's own documents, with citations. | LlamaIndex + pgvector + BGE embeddings answer from at least 20 bank documents; answers are cited; the system says ΓÇ£I don't knowΓÇ¥ when it does not have grounds to answer. |
...
### Track D ΓÇö Documents & Evaluation (FR-D.x)
...
| **FR-D.3** | The system shall support adversarial (ΓÇ£red-teamΓÇ¥) testing of prompts and produce a scorecard per demo. | A scorecard exists for each of the four demos, plus a consolidated build-vs-buy verdict. |

### Track E ΓÇö Observability & Guardrails (FR-E.x)
...
| **Evaluatability** | Every demo must pass through a shared evaluation harness (Ragas, promptfoo) producing accuracy, groundedness, latency, and cost metrics ΓÇö claims must be backed by numbers. |
...
* **Main flow:**
  1. Ingest ΓëÑ20 documents via LlamaIndex into pgvector using BGE embeddings.
...
* **Postcondition:** The user receives a cited, grounded answer or an honest ΓÇ£I don't knowΓÇ¥, and the interaction is fully traced.
...
### UC-5: Control Tower ΓÇö Tracing, PII & Guardrails (Track E)
* **Actor:** Risk & compliance reviewer (illustrative end user); Intern E (builder)
* **Goal:** Answer the question ΓÇ£can we run this safely under SAMA / PDPL?ΓÇ¥ for every other demo, since this decides whether anything can ever ship.
* **Preconditions:** Demos from Tracks AΓÇôD are producing traceable runs.
* **Main flow:**
  ...
  4. Run a red-team set of ΓëÑ20 adversarial prompts (via Garak) and report the block rate.
...
* Only synthetic or public sample data may be used ΓÇö nothing real, nothing personal.
* The project duration is fixed at six weeks (Jun 23 ΓÇô Aug 04, 2026), ending on a fixed Demo Day.
...
| When | What happens |
| :--- | :--- |
| **Week 1** | **Whole team:** stand up the shared repo, Docker/docker-compose, a local LLM via Ollama, Postgres + pgvector/Qdrant, and Langfuse tracing. Fork the shared ΓÇ£hello agentΓÇ¥ template. Confirm everything starts with one command. Tracks A, B, and C begin their first tasks (A1, B1, C1). |
...
| **Week 4** | **A:** quantize a model for modest hardware (A3). **B:** add the LangGraph human-checkpoint flow (B3, into Week 5). **C:** wrap retrieval in a chat UI with memory (C3, into Week 5). **D:** build the shared evaluation harness (D2, into Week 4ΓÇô5). **E:** finish PII/guardrails (E2). |
...
* Assign Interns AΓÇôE to their tracks.
...
* **Run before you build** ΓÇö get the shared template working end-to-end before changing anything; confidence comes from seeing it work first.
* **Copy, then change one thing** ΓÇö fork the template, swap one piece, and observe what breaks; this teaches more than reading documentation alone.
* **Read the traces first** ΓÇö when stuck, check Langfuse to see exactly what the model received and did before searching elsewhere.
* **Pair up, especially in Week 1** ΓÇö nobody should fight Docker alone; setting up the base together builds a shared mental model.
* **Teach it back on Fridays** ΓÇö explaining your layer to the other four interns is both a comprehension check and how the team stays aligned.
* **Always ask the banker question** ΓÇö for everything built, ask ΓÇ£would a bank let this near a customer?ΓÇ¥; if the answer is no, that becomes the next task.

---

## Definition of Done
A piece of work on this project is only considered complete when all of the following hold:
* It lives in the shared repository and starts via the single shared `docker-compose` command ΓÇö if it only runs on one laptop, it is not done.
...
* It ends with a stated verdict ΓÇö Build / Buy / Partner / Park for the demo, and Adopt / Trial / Hold for each tool used.

---

## Success Criteria
By Demo Day (August 4, 2026), the project is considered successful if:
* All five interns have a working, demoable system in their track ΓÇö not a notebook or slide mock-up.
...
| :--- | :--- | :--- |
| **Local, self-hosted models** | Where does our customer data go? | Nowhere ΓÇö it stays on our own hardware. |
| **Open source, no vendor** | Are we locked in to someone? | No ΓÇö we own the code and can audit it. |
| **Observability (Langfuse)** | Can we prove what the AI did? | Yes ΓÇö every run is traced and reviewable. |
...
| **Docker / on-prem packaging** | Can it run in our data centre? | Yes ΓÇö the same container, on our own metal. |

---

## Closing Note
The intended payoff of this sprint is not just five working demos: it is a team that has hands-on comfort with the entire open-source, on-prem AI stack ΓÇö models, agents, retrieval, evaluation, and guardrails ΓÇö and the habit of asking, for everything they build, whether a bank would actually let it near a customer.
